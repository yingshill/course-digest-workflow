...
- **C1.** Must follow shared vocabulary rules — zero drift across all 4 source DBs — ✅ Managed (L1 S5: 3 Course Digest drift items found and fixed; pre-existing drift in older DBs deferred to separate System Audit — see §8 Q4)
...
| **R3.** Shared vocabulary audit misses an option in one DB | Low | High | Run SQL diff across all 4 source DBs as final build step | Triggered — mitigated (L1 S5 found 3 drift items in Course Digest; all fixed. Pre-existing drift in older DBs deferred.) |
